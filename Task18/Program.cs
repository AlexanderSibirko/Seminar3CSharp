...
// //Вариант 1, тупо перебор, но работает только на вписанное в код утверждение => плохо не применимо => фу-фу-фу
// bool solveTester()
// {
...
// OBSOLETE //написал метод получепния начальных условий исходя из кол-ва переменных
...
bool[] baseBoolArr(int paramTotalCount, int paramNum) //создатель базовых исходов входные данные: общее кол-во переменных, норме переменной
{
    ...
    return result;
} //???Мысли про метод: можно переписать на метод возвращающий массив массивов и получать только кол-во переменных

bool[] X = baseBoolArr(2, 1);
bool[] Y = baseBoolArr(2, 2);

Console.WriteLine(printArr(X)); //debugging result check
Console.WriteLine(printArr(Y)); //debugging result check
...
Console.WriteLine(isRight(NOT(OR(X, Y)), AND(NOT(X), NOT(Y))));
